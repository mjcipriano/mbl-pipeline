...
my $check_read_existance_h = $dbh->prepare("select read_id, read_name from reads where read_name = ?");
...
if (0) 
{
	...
	if (-f $assembly_reads_file) {
		...
		while (<READS>)
		{
			...
			$check_read_existance_h->execute($reads_data[0]);
			if($check_read_existance_h->rows == 0)
			{
