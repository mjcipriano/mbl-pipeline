...
sub new 
{
	...
	{
		$self->{DRIVER} = "mysql";
		$self->{HOSTNAME} = "scorpius.ucdavis.edu";
		$self->{PORT} = "3306";
		$self->{USER} = "gmod";
		$self->{PASSWORD} = "gmodgmod123!";
	}
	...
	$self->{GO_DATABASE} = "go";
	$self->{WEB_SERVER} = "scorpius.ucdavis.edu";
	$self->{WEB_HOME} = '/gmod/cgi-bin/';
	$self->{GBROWSE_CGI_DIR} = '/gmod/cgi-bin/gb2/';
	...
	$self->{MBLPIPE_DIR} = '/data/gmod/mbl-pipeline';
	$self->{MBLDATA_DIR} = '/data/gmod/data';
	$self->{GMODWEB_DIR} = '/data/gmod/mblweb-gmod/';
	$self->{GMODWEB_HTML_TMP_SYS_DIR} = $self->{GMODWEB_DIR} . 'html/temp';
	$self->{GMODWEB_HTML_TMP_WEB_DIR} = "/gmod/temp";
...
sub set_queries
{
	...
	AND st.id = br.sequence_type_id AND db.name IN ('nr','swissprot','Pfam_ls','mitop', 'RefEuks', 'RefTax', 'refseq_protein') AND br.idname = ? AND br.evalue < ?  AND (br.description not like '%ATCC %' OR br.description like '%gb|%') AND br.db = db.id ORDER BY br.db, br.evalue ";
	...
	AND st.id = br.sequence_type_id AND algorithms.id = br.algorithm AND db.name IN ('Pfam_ls', 'interpro', 'tmhmm') AND br.idname = ? AND (br.evalue < ? OR br.evalue is NULL)  AND (br.description not like '%ATCC %' OR br.description like '%gb|%') AND br.db = db.id ORDER BY br.query_start, br.db, br.algorithm, br.query_start, br.evalue";
	...
	AND st.id = br.sequence_type_id AND algorithms.id = br.algorithm AND db.name IN ('Pfam_ls', 'interpro') AND br.idname = ? AND (br.evalue < ? OR br.evalue is NULL)  AND (br.description not like '%ATCC %' OR br.description like '%gb|%') AND br.db = db.id AND br.description != 'seg' ORDER BY br.db, br.algorithm, br.query_start, br.evalue ";
...
WHERE reads.read_name = ?";
	...
	$query->{'read_sequence_trim'} = "select substring(rb.bases, ra.first_base_of_trim, ra.read_len_trim) as bases from reads_bases rb, reads_assembly ra WHERE rb.read_name = ?";
	...
	$query->{'get_taxid_gi'} = "select ncbi_taxon_id as taxid from gmoddb.gitotaxid where gi = ?";
	...
	# Lists
	$query->{'get_lists'} = "select list_id, list_name, list_description, date_created, user.user_name from list, user where list.user_id = user.id order by list_id";
	$query->{'get_lists_by_keyword'} = "select list_id, list_name, list_description, date_created, user.user_name from list, user where list.user_id = user.id AND ( locate(lower(?), lower(list_description)) > 0 OR locate(lower(?), lower(list_name)) > 0  OR locate(lower(?), lower(user.user_name)) > 0 ) order by list_id";
	$query->{'get_list'} = "select list_id, list_name, list_description, date_created, user.user_name from list, user where list.user_id = user.id AND list_id = ?";
	$query->{'get_list_orfs'} = "select orfid, list_id, annotation, description, date_added, score, user.user_name from orfid2list, user where orfid2list.user_id = user.id AND list_id = ? order by score, annotation, orfid";
	$query->{'get_orf_lists'} = "select list.list_id, list.list_name, list.list_description, list.date_created, list.user_id, user.user_name, orfid2list.annotation, orfid2list.description, orfid2list.date_added as orfid_date, orfid2list.user_id as orfid_user_id, orfid2list.score from list, orfid2list, user where list.user_id = user.id AND list.list_id = orfid2list.list_id AND orfid2list.orfid = ? order by list.list_id";
$query->{'get_orf_list_info'} = "select list.list_id, list.list_name, list.list_description, list.date_created, list.user_id, user.user_name, orfid2list.orfid, orfid2list.annotation, orfid2list.description, orfid2list.date_added as orfid_date, orfid2list.user_id as orfid_user_id, orfid2list.score from list, orfid2list, user where list.user_id = user.id AND list.list_id = orfid2list.list_id AND orfid2list.orfid = ? AND list.list_id = ?";
	$query->{'get_orf_list_count'} = "select count(DISTINCT list.list_id) as count_lists from list, orfid2list where list.list_id = orfid2list.list_id AND orfid2list.orfid = ?";

	$query->{'delete_orf_from_list'} = "delete from orfid2list where list_id = ? AND orfid = ?";
	$query->{'insert_orf_into_list'} = "insert into orfid2list (list_id, orfid, annotation, description, date_added, user_id, score) VALUES (?, ?, ?, ?, ?, ?, ?)";
	$query->{'insert_list'} = "insert into list ( list_id, list_name, list_description, date_created, user_id) VALUES (?, ?, ?, ?, ?)";
	$query->{'update_orf_in_list'} = "update orfid2list set annotation = ?, description = ?, date_added = ?, user_id = ?, score = ? where orfid = ? AND list_id = ?";
	$query->{'update_list'} = "update list set list_name = ?, list_description = ?, date_created = ?, user_id = ? where list_id = ?";
	$query->{'delete_list'} = "delete from list where list_id = ?";
...
sub check_list_exists
{
	my $self = shift;
	my $list_id = shift;
	my $user_id = shift;

        my $sth = $self->query('get_list');
        $sth->execute($list_id);
	if($sth->rows > 0)
	{
		return 1;
...
sub get_orf_lists
{
	...
	my @orf_lists = ();
        ...
        my $sth = $self->query('get_orf_lists');
        $sth->execute($orfid);
...
sub get_orf_lists_info
{
	...
	{
	        my $sth = $self->query('get_orf_list_info');
	        $sth->execute($orfid, $list_id);
		return $sth;
	}
...
sub get_orf_lists_info_hashref
{
	my $self = shift;
	my $orfid = shift;
	my $list_id = shift;
...
sub get_orf_list_count
{
	...
	my @orf_lists = ();
        ...
        my $sth = $self->query('get_orf_list_count');
        $sth->execute($orfid);

	return $sth->fetchrow_hashref->{count_lists};
}
...
sub get_list_orfs
{
	...
	my @orf_lists = ();
        ...
        my $sth = $self->query('get_list_orfs');
        $sth->execute($list_id);
	my @orfs = ();
	while(my $row = $sth->fetchrow_hashref)
	{
...
sub get_lists_by_keyword
{
	...
	my @lists = ();
        ...
        my $sth = $self->query('get_lists_by_keyword');
        $sth->execute($keyword, $keyword, $keyword);

	while(my $row = $sth->fetchrow_hashref)
	{
		push(@lists, $row->{list_id});
	}
	return \@lists;
...
sub list_link
{
	my $self = shift;
	my $listid = shift;
	return '<a href="' . $self->organism_home() . "?page=showlist&list=$listid" . '">' . $listid . '</a>';
}

sub delete_orf_from_list
{
	my $self = shift;
	my $list_id = shift;
	my $orfid = shift;

	if($self->check_list_exists($list_id) && defined($orfid))
	{
		my $sth = $self->query('delete_orf_from_list');
		$sth->execute($list_id, $orfid);
		return 1;
	} else
	{
		return 0;
...
sub insert_orf_into_list
{
	...
	if($self->check_list_exists($list_id) && $self->check_orf_existance($orfid))
	{
		my $sth = $self->query('insert_orf_into_list');
		$sth->execute($list_id, $orfid, $annotation, $description, $date_added, $user_id, $score);
		return 1;
	} else
	{
		return 0;
...
sub insert_list
{
	...
	my $sth = $self->query('insert_list');
	$sth->execute(undef, $list_name, $description, $date_added, $user_id);
	
	return $sth->{'mysql_insertid'};
}

sub update_orf_in_list
{
	my $self = shift;
	...
	my $sth = $self->query('update_orf_in_list');
	$sth->execute($annotation, $description, $date_added, $user_id, $score, $orf_id, $list_id);
...
sub update_list
{
	...
	my $list_id = shift;
	my $list_name = shift;
	my $list_description = shift;
	my $date_added = shift;
	my $user_id = shift;

	my $sth = $self->query('update_list');
	$sth->execute($list_name, $list_description, $date_added, $user_id, $list_id);
...
sub get_read_table
{
	...
	$ret_val = table( { -border=>'1', -width=>'100%'}, 
				...
				TR(
					th('Untrimed Length'), td($read->{read_len_untrim})
				),
				TR(
				...
				TR(
					th('Contig'), td($read->{contig_number})
				),
				TR(
					th('Supercontig'), td($super_id)
				),
				TR(
...
sub get_genbank_seq
{
	...
	if(!defined($db))
	{
        ...
        `/bioware/blast/bin/fastacmd -d $db -s $gi > /tmp/$gi`;
...
sub get_name_from_taxon_id
{
	my $self = shift;
        my $taxid = shift;

	my $seqdb_dbh = $self->shared_dbh();
...
=head2 get_parent_taxon_id

 Title   : get_parent_taxon_id
...
=head2 get_parent_rank_taxid

 Title   : get_parent_rank_taxid
...
sub get_parent_rank_taxid
{
	...
	my $taxid = shift;
        ...
        if(lc($self->get_node_rank($taxid)) eq lc($rank_name))
        {
                return $taxid;
        }

        # Ok, I'm not the rank I want, so now just keep calling my parent until either it is undef, or my rank
        return  $self->get_parent_rank_taxid($rank_name, $self->get_parent_taxon_id($taxid) );
